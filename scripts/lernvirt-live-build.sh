...
# ein bootfähiges Live-ISO auf Basis eines
# Ubuntu Server Live ISO.
#
# Eigenschaften:
# - echtes Live-System mit casper
# - generisches DHCP-Netz via netplan
# - Installer/Subiquity neutralisiert
# - SSH immer aktiviert
# - vorhandener User "ubuntu" bleibt erhalten
# - zusätzlicher SSH-Key für "ubuntu" wird gesetzt
########################################
...
cleanup() {
  ...
  for p in dev/pts dev proc sys run; do
    mountpoint -q "$ROOTFS/$p" && umount -lf "$ROOTFS/$p"
  done
...
require_root
for cmd in rsync xorriso mksquashfs mount umount chroot find sed awk cp; do
  require_cmd "$cmd"
done
...
echo "==> Installer-Reste und Netzkonfiguration bereinigen"
rm -f "$ROOTFS/etc/netplan/"*.yaml || true
rm -f "$ROOTFS/etc/cloud/cloud.cfg.d/"*subiquity* || true
rm -f "$ROOTFS/etc/cloud/cloud-init.disabled" || true

echo "==> Generische DHCP-Konfiguration schreiben"
...
if ! grep -Fqx "$LEERNCLOUD_PUBKEY" "$ROOTFS/$USER_HOME/.ssh/authorized_keys"; then
  printf '%s\n' "$LEERNCLOUD_PUBKEY" >> "$ROOTFS/$USER_HOME/.ssh/authorized_keys"
fi
...
for svc in \
  ...
  serial-subiquity@ttyS0.service
do
  ln -sf /dev/null "/etc/systemd/system/$svc" || true
done

# Cloud-init im Live-System deaktivieren
touch /etc/cloud/cloud-init.disabled

# SSH immer aktiv
systemctl enable ssh || true

# Host Keys beim Boot neu erstellen
...
# sauberes resolv.conf im laufenden System
...
echo
echo "QEMU-Test:"
echo "  qemu-system-x86_64 -m 8192 -enable-kvm -cpu host -cdrom \"$OUT_ISO\" -boot d"
echo
echo "Serieller Test:"
